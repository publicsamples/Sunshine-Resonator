...
struct _DiodeLPF final : public ::faust::dsp {
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp2 = 0.5f * (fRec1[1] * fTemp0 / fTemp1) + fRec2[1];
			...
			float fTemp9 = fRec4[1] + fTemp8;
			float fTemp10 = (1.0f - 0.5f * (fTemp0 / fTemp7)) * fTemp0 + 1.0f;
			float fTemp11 = std::max<float>(-1.0f, std::min<float>(1.0f, 1e+02f * float(input0[i0])));
			float fTemp12 = 17.0f - 9.7f * std::pow(fRec5[0], 1e+01f);
			float fTemp13 = _DiodeLPF_faustpower2_f(fTemp0);
			float fTemp14 = ((0.5f * (fTemp9 * fTemp0 / fTemp10) + fTemp6) / fTemp7 + (1.5f * fTemp11 * (1.0f - 0.33333334f * _DiodeLPF_faustpower2_f(fTemp11)) - fSlow1 * ((0.0051455377f * (fTemp9 * _DiodeLPF_faustpower3_f(fTemp0) / (fTemp10 * fTemp3 * fTemp7)) + 0.0411643f * fRec1[1] + 0.02058215f * fTemp4 + 0.02058215f * fTemp8) * fTemp12 / fTemp1)) * (0.5f * (fTemp13 / (fTemp10 * fTemp7)) + 1.0f) / (fSlow2 * (_DiodeLPF_faustpower4_f(fTemp0) * fTemp12 / (fTemp10 * fTemp1 * fTemp3 * fTemp7)) + 1.0f) - fRec4[1]) * fTemp0 / fTemp1;
			float fTemp15 = (0.5f * ((fRec4[1] + fTemp14) * (0.25f * (fTemp13 / (fTemp3 * fTemp7)) + 1.0f) + (0.5f * fTemp8 + fTemp2) / fTemp3) - fRec3[1]) * fTemp0 / fTemp1;
			float fTemp16 = (0.5f * ((fTemp15 + fRec3[1]) * (0.25f * (fTemp13 / (fTemp1 * fTemp3)) + 1.0f) + (fRec1[1] + fTemp5) / fTemp1) - fRec2[1]) * fTemp0 / fTemp1;
			float fTemp17 = (0.5f * (fTemp16 + fRec2[1]) - fRec1[1]) * fTemp0 / fTemp1;
			float fRec0 = fTemp17 + fRec1[1];
			fRec1[0] = 2.0f * fTemp17 + fRec1[1];
			fRec2[0] = 2.0f * fTemp16 + fRec2[1];
			fRec3[0] = 2.0f * fTemp15 + fRec3[1];
			fRec4[0] = fRec4[1] + 2.0f * fTemp14;
