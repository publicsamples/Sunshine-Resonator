...
#include "src/DiodeLPF.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct DiodeLPFMetaData {
		SN_NODE_ID("DiodeLPF");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using DiodeLPF = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _DiodeLPF, DiodeLPFMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
